...
def extract_places(batch):
    prompt = "Extract places mentioned in the following Bible verses using canonical names.\n"
    prompt += "Return JSON: [{\"id\": ..., \"places\": [{\"name\": ..., \"aliases\": [...]}, ...]}, ...]\n\n"
    prompt += "Verses:\n"
    for verse in batch:
...
for i in tqdm(range(num_batches), desc="Extracting Places"):
    batch = verses[i * BATCH_SIZE:(i + 1) * BATCH_SIZE]
    try:
        result = extract_places(batch)
